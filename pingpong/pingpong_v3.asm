...
; Check for win conditions and reset the game state appropriately.
;win_state_check:
;    mov A, position         ; Load the current ball position into accumulator.
;    cjne A, #0Bh, not_p2_win; Check if Player 2 has won (position 11).
;    ; Player 2 wins
;    mov position, #1        ; Reset position for the start of the next game, indicating Player 1 starts.
;    jmp win                 ; Go to the win routine, stopping game progress and displaying win state.
;    
;not_p2_win:
;    cjne A, #0, not_p1_win  ; Check if Player 1 has won (position 0).
;    ; Player 1 wins
...
win_state_check:
    ; Check the winning conditions for both sides.
    mov A, position       ; Copy the current position to A for comparison.
    jz p1_wins            ; If position is 0, player 1 wins.
    jz p2_wins            ; If position is 11 (0Bh), player 2 wins.
    ret                   ; If neither condition is met, return without change.

p1_wins:
    mov position, #1      ; Reset position for player 1's win.
    jmp game_over               ; Jump to win handling routine.
    
p2_wins:
    mov position, #10     ; Reset position for player 2's win.
    jmp game_over               ; Jump to win handling routine.
