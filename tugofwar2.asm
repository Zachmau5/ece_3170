...
game_over:
    ; Flash all LEDs for a few times
    mov R7, #5              ; Number of times to flash
flash_loop:
    orl P1, #0FFH           ; Turn on all LEDs
    call delay_200ms        ; Delay
    anl P1, #00H            ; Turn off all LEDs
    call delay_200ms        ; Delay
    djnz R7, flash_loop     ; Repeat flash

    ; Reset the game state
    call initialize_state   ; Reset to initial state
...
; Delay Routine (approximately 200ms)
delay_200ms:
    mov R4, #13            ; Additional outer loop for scaling
outer_loop:
    mov R2, #50
loop1_200ms:
    mov R3, #200
