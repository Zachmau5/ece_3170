...
// Global score array
unsigned int scores[3] = {0, 0, 0};  // scores[0] for Player 1, Player 2, and High score
...
void drawBricks() {
    ...
    // Decide which brick array to use based on the current player
    char (*currentBricks)[16] = (currentPlayer == 1) ? bricksPlayer1 : bricksPlayer2;

    for (i = 0; i < 4; ++i) {
        for (j = 0; j < 16; ++j) {
            if (currentBricks[i][j] == 1) {  // Only draw if the brick is active
	...
	for (i = 0; i < 4; i++) {		//row of bricks
        for (j = 0; j < 16; j++) {	//col of bricks
            ...
            if (ball_x >= brick_x_start && ball_x <= brick_x_end &&
                ...
                if (currentBricks[i][j] == 1) {  // Brick is active
                    currentBricks[i][j] = 0;  // Deactivate the brick
                    y_vel = -y_vel;  // Reverse the ball's vertical velocity
                    // Update score when a brick is hit
                    scores[currentPlayer-1] += 1;  // Increase score, assuming Player 1 is playing
                    ...
                    if (allCleared) {
						...
						ball_x=starting_position_y;;
						ball_y=starting_position_x;
						pot_avg = SCREEN_WIDTH / 2;  // Assuming you start with the paddle in the middle
    					drawPaddle(); 
						delay(2000);
						x_vel=0;
						y_vel= 1;
...
void pregame() {
    ...
    col = 6;
    disp_char(row, col, 'M');
    disp_char(row, col + 6, 'O');
    disp_char(row, col + 12, 'D');
    ...
    if (game_mode == 0) {
        ...
        disp_char(row, col + 24, 'P');
        disp_char(row, col + 30, 'l');
        disp_char(row, col + 36, 'a');
        disp_char(row, col + 42, 'y');
        disp_char(row, col + 48, 'e');
        disp_char(row, col + 54, 'r');
    } else {
...
void drawServe(){
    ...
    // Display "P1" and P1's score
    disp_char(row, col, 'S');
    disp_char(row, col + 6, 'E');
	disp_char(row, col + 12, 'R');
	disp_char(row, col + 18, 'V');
    disp_char(row, col + 24, 'I');
	disp_char(row, col + 30, 'N');
	disp_char(row, col + 36, 'G');
