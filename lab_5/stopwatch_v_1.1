...
NOT_C:
    ...
    MOV     A, count              ; Prepare the snapshot of the count for sending over serial
    ANL     A, #0Fh               ; Mask off any high bits, leaving only the second
    MOV     snapshot_tenth, A
    MOV     A, count              ; Get the count again to extract tenths of a second
    SWAP    A
    ORL     A, #30h
...
T2_INT:
    ...
    CALL    DISP_LED              ; Update the LED display with the new count
T2_SKIP:
    RETI
...
DISP_LED: 
    MOV     A, count              ; Move the count into A
    MOV     B, A                  ; Copy count into B for separate manipulation
    ANL     B, #0F0h              ; Isolate seconds (upper nibble)
    SWAP    B                     ; Swap nibbles to align for P3.4-P3.7
    ANL     B, #F0h               ; Ensure only the seconds are left
    ANL     A, #0Fh               ; Isolate tenths (lower nibble)
    ORL     B, A                  ; Combine seconds and tenths
    MOV     P3, B                 ; Output combined BCD to P3
    RET
